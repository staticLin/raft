	...
	for time.Since(t0).Seconds() < 20 {
		...
		for si := 0; si < cfg.n; si++ {
			...
			if rf != nil {
				...
				if ok {
					index = index1
					DPrintf("expect cmd:%d index = %d", cmd, index)
					break
				}
			}
		}
			...
			for time.Since(t1).Seconds() < 2 {
				...
				if nd > 0 && nd >= expectedServers {
					...
					if cmd2, ok := cmd1.(int); ok && cmd2 == cmd {
						...
						DPrintf("expect %d, yes cmd = %d", cmd, cmd1.(int))
						return index
					}
					DPrintf("expect %d, but cmd = %d", cmd, cmd1.(int))
				}
				DPrintf("expect index %d cmd = %d, but real cmd = %d, nd = %d, expectedServers = %d",
					index, cmd, cmd1.(int), nd, expectedServers)
